...
const getEnrolledUnderBadgeClass = (enrolled_under: string) => {
  switch (enrolled_under) {
    case "Undergraduate":
      return "bg-emerald-100 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 px-3 py-1 text-medium";

    case "Graduate":
      return "bg-blue-100 text-blue-700 border border-blue-200 hover:bg-blue-100 px-3 py-1 text-medium";

    case "School of Law":
      return "bg-amber-100 text-amber-700 border border-amber-200 hover:bg-amber-100 px-3 py-1 text-medium";
...
const getSemesterBadgeClass = (semester: string) => {
    switch (semester) {
        case 'First Semester':
            return 'bg-emerald-100 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 px-4 py-1';

        case 'Second Semester':
            return 'bg-blue-100 text-blue-700 border border-blue-200 hover:bg-blue-100 px-4 py-1';

        case 'Summer':
            return 'bg-amber-100 text-amber-700 border border-amber-200 hover:bg-amber-100 px-4 py-1';
...
export const columns = columnHelper.columns([
  ...
  columnHelper.accessor("email", {
    ...
    cell: ({ row }) => (
      <div className="text-center text-slate-600">{row.getValue("email")}</div>
    ),
  }),
  ...
  columnHelper.accessor("enrolled_under", {
    ...
    cell: ({ row }) => {
        ...
        return (
          <div className="flex justify-center">
            <Badge className={getEnrolledUnderBadgeClass(enrolledUnder)}>
              {enrolledUnder}
            </Badge>
          </div>
        );
  ...
  columnHelper.accessor("sy_last_attended", {
    ...
    cell: ({ row }) => (
      <div className="text-center text-slate-600">{row.getValue("sy_last_attended")}</div>
    ),
  }),
