...
export default function EditTransaction({ record, studentNames, authUserName }: Props) {
  ...
  const computedAmount = data.ar_payment === 'AR' ? parsedUnits * parsedRate : Number(data.amount || 0);
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setData('semester', value === 'Summer' ? 'Summer' : value === '2nd Sem.' ? 'Second Semester' : 'First Semester');
                  }}
                  className="w-full rounded-md border border-[#CFE3FF] bg-white px-3 py-2 text-sm text-[#334E68] focus:outline-none focus:ring-2 focus:ring-[#0F6FFF]"
                >
              ...
              <div>
                ...
                <Input value={data.semester} readOnly className="bg-[#F8FAFC]" />
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  step="0.01"
                  min="0"
                  value={data.ar_payment === 'AR' ? computedAmount.toFixed(2) : data.amount}
                  readOnly={data.ar_payment === 'AR'}
                  onChange={(e) => setData('amount', e.target.value)}
                  className={`${data.ar_payment === 'AR' ? 'bg-[#F8FAFC]' : ''} ${errors.amount ? 'border-red-400' : ''}`}
                />
