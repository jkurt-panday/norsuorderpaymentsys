...
export function DonutChartCard({
  ...
  const { chartData, config } = useMemo(() => buildCategoryChartData(data), [data])
  const total = useMemo(() => data.reduce((sum, d) => sum + d.value, 0), [data])

  return (
    <Card className={`flex flex-col rounded-2xl border border-slate-200/70 bg-white shadow-sm ${className ?? ""}`}>
      ...
      <CardContent className="flex-1 pb-0">
        {data.length === 0 ? (
          <div className={`flex ${size} items-center justify-center text-sm text-slate-400`}>
            {emptyMessage}
          </div>
        ) : (
          <ChartContainer config={config} className={`mx-auto aspect-square ${size}`}>
            <PieChart>
                      ...
                      if (viewBox && "cx" in viewBox && "cy" in viewBox) {
                        return (
                          <text x={viewBox.cx} y={viewBox.cy} textAnchor="middle" dominantBaseline="middle">
                            <tspan x={viewBox.cx} dy="-1em" className="fill-foreground text-2xl font-bold">
