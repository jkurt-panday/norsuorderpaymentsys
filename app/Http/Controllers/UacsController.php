...
class UACSController extends BaseResourceController
{
    ...
    protected function modifyIndexQuery(Builder $query, Request $request): Builder
    {
        ...
        return $query
            ->select('*')
            ->selectRaw("(SELECT COUNT(*) FROM {$table} AS t2 WHERE t2.id <= {$table}.id) as display_number");
    }
    ...
    public function store(UacsRequest $request)
    {
        try {
            ...
            DB::rollBack();
            Log::error('Failed to create UACS: ' . $e->getMessage(), ['request' => $request->validated()]);

            return back()->withInput()->with('error', 'Failed to create UACS. Please try again.');
        }
    }
    ...
    public function update(UacsRequest $request, Uacs $uacs)
    {
        try {
            ...
            return back()->withInput()->with('error', 'Failed to update UACS. Please try again.');
        }
    }
